...
(define-subwidget (stethoscope-ctl layout) (q+:make-qhboxlayout stethoscope-ctl)
  (let ((io-menu (q+:make-qmenu)))
    (setf audio-in-action (q+:make-qaction "Audio in" io-menu))
    (setf audio-out-action (q+:make-qaction "Audio out" io-menu))
    (setf bus-action (q+:make-qaction "Bus" io-menu))
    (q+:add-action io-menu audio-in-action)
    (q+:add-action io-menu audio-out-action)
    (q+:add-action io-menu bus-action)
    (q+:set-menu io-button io-menu)
    (q+:set-text io-button "Audio in"))
  (let ((view-menu (q+:make-qmenu)))
    (setf tracks-action (q+:make-qaction "Tracks" view-menu))
    (setf overlay-action (q+:make-qaction "Overlay" view-menu))
    (setf xy-action (q+:make-qaction "X/Y" view-menu))
    (q+:add-action view-menu tracks-action)
    (q+:add-action view-menu overlay-action)
    (q+:add-action view-menu xy-action)
    (q+:set-menu mode-button view-menu)
    (q+:set-text mode-button "Tracks"))
  (q+:add-widget layout bus-num-box)
  (q+:add-widget layout num-chans-box)
  (q+:add-widget layout io-button)
  ...
  (q+:set-fixed-width io-button 95)
...
(define-slot (stethoscope audio-in-action) ()
  (declare (connected
            (audio-in-action (steth-ctl stethoscope))
            (triggered)))
  (setf (redraw? stethoscope) nil)
  (setf (io-mode stethoscope) :audio-in)
  (q+:set-text (io-button (steth-ctl stethoscope)) "Audio in")
...
(define-slot (stethoscope audio-out-action) ()
  (declare (connected
            (audio-out-action (steth-ctl stethoscope))
            (triggered)))
  (setf (redraw? stethoscope) nil)
  (setf (io-mode stethoscope) :audio-out)
  (q+:set-text (io-button (steth-ctl stethoscope)) "Audio out")
  (restart-stethoscope-dsp stethoscope)
  (setf (redraw? stethoscope) t)
  (q+:repaint (steth-view-pane stethoscope)))

(define-slot (stethoscope bus-action) ()
  (declare (connected
            (bus-action (steth-ctl stethoscope))
            (triggered)))
  (setf (redraw? stethoscope) nil)
  (setf (io-mode stethoscope) :bus)
  (q+:set-text (io-button (steth-ctl stethoscope)) "Bus")
  (restart-stethoscope-dsp stethoscope)
  (setf (redraw? stethoscope) t)
  (q+:repaint (steth-view-pane stethoscope)))
